class ChecksController < ApplicationController
  ...
  # GET /checks/1.json
  def show
    cmd3 = "rrdtool graph #{@check.rrd_png}                                 \
         --start #{Time.now.to_i - 60 * 60} --end #{Time.now.to_i}               \
         DEF:myspeed=#{@check.rrd_path}:speed:AVERAGE              \
  ...
  private
    ...
    def check_params
      params.require(:check).permit(:name, :url)
    end
end
